...
def test_dataset_manifest_diff(tmp_path):
    root = tmp_path/"d"; (root/"a").mkdir(parents=True)
    ...
    (root/"a"/"x2.txt").write_text("gone"); (root/"a"/"x2.txt").unlink()
    run(["scripts/datasets/manifest.py","--root",str(root),"--out",str(tmp_path/"m2.json")])
    r = run(["scripts/datasets/diff_manifest.py","--old",str(tmp_path/"m1.json"),"--new",str(tmp_path/"m2.json")])
    out = r.stdout
