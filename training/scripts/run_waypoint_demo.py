...
def synth(n:int=120, dt:float=0.02):
    ...
    for i in range(n):
        t=i*dt
        x=10*math.cos(0.1*i); y=10*math.sin(0.1*i); z=5+0.2*math.sin(0.05*i)
        vx=-1.0*math.sin(0.1*i); vy= 1.0*math.cos(0.1*i); vz=0.01*math.cos(0.05*i)
...
def main():
    ...
    args=ap.parse_args()
    n=max(10,int(args.sim-seconds/args.dt))
