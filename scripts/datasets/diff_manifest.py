...
def idx(m):
    return {f["relpath"]: f.get("sha256") for f in m.get("files", [])}
...
def main():
    ...
    for x in added:
        print(f"[diff] ADDED    {x}")
    for x in removed:
        print(f"[diff] REMOVED  {x}")
    for x in modified:
        print(f"[diff] MOD      {x}")
    if not (added or removed or modified):
        print("[diff] no changes")
    if (added or removed or modified) and ap.parse_args().fail_on_change:
        sys.exit(3)
