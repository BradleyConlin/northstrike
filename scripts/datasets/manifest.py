...
def main():
    ...
    for dp,_,files in os.walk(root):
        for fn in files:
            ...
            total+=1
            sz=p.stat().st_size
            rec={"relpath": str(p.relative_to(root.resolve())), "bytes": sz, "mtime": int(p.stat().st_mtime)}
            if sz<=max_bytes:
                rec["sha256"]=sha256(p); hashed+=1
            else:
                rec["sha256"]=None; rec["skipped_size"]=True
            items.append(rec)
    man={"meta":{"tool":"datasets/manifest.py","ts":int(time.time()),"root":str(root),
                 "total":total,"hashed":hashed,"max_bytes":max_bytes,"exts":args.exts},
         "files":items}
