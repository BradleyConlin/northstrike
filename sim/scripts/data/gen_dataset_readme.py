...
def main():
    ...
    if bytes_total:
        ...
        lines.append(
            "  - Quick verify sample: `python scripts/data/verify_hashes.py --root ROOT --csv ROOT/checksums/checksums.csv --sample 200`"
        )
        lines.append(
            "  - Full verify: `python scripts/data/verify_hashes.py --root ROOT --csv ROOT/checksums/checksums.csv --sample 0`"
        )
    else:
        lines.append(
    ...
    lines.append("\n## Repro commands")
    lines.append("```bash")
    lines.append("# Smoke & integrity")
    lines.append(f"python scripts/data/verify_depth_manifest.py --dataset_root '{root}'")
    if bytes_total:
        lines.append(
            f"python scripts/data/verify_hashes.py --root '{root}' --csv '{root}/checksums/checksums.csv' --sample 200"
        )
    lines.append("```")
