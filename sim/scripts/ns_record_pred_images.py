...
class PredRecorder(Node):
    def __init__(self, out_dir, pred_topic, color_topic, stride, limit, max_depth_m):
        ...
        (self.out / "pred").mkdir(parents=True, exist_ok=True)
        (self.out / "pred_color").mkdir(exist_ok=True)
        ...
        self.get_logger().info(
            f"Recording to {self.out} (pred in mm, color JPG). stride={self.stride} limit={self.limit or '∞'}"
        )
    ...
    def cb_color(self, msg: Image):
        ...
        cv2.imwrite(
            str(self.out / "pred_color" / f"{name}.jpg"), img, [int(cv2.IMWRITE_JPEG_QUALITY), 92]
        )
...
def main():
    ...
    rclpy.init()
    node = PredRecorder(
        args.out, args.pred_topic, args.color_topic, args.stride, args.limit, args.max_depth_m
    )
    try:
        rclpy.spin(node)
