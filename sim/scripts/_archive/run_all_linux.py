...
def rotate_logs(dir_path: Path, keep: int = 10):
    try:
        files = sorted(dir_path.glob("drone_*.txt"), key=lambda p: p.stat().st_mtime, reverse=True)
        for f in files[keep:]:
...
parser.add_argument(
    "--sleep-between", type=float, default=2.0, help="Seconds to sleep between stages"
)
parser.add_argument(
    "--detach", type=str, default="false", help="true/false - run PX4 in background"
)
# passthrough to PX4 launcher:
parser.add_argument("--model", type=str, default="x500", help="PX4/GZ model (default: x500)")
parser.add_argument(
    "--offset", type=float, default=2.0, help="Spacing (m) between drones (default: 2.0)"
)
...
def main():
    ...
    log.info(
        "Drones: %d | Headless: %s | QGC: %s | Detach: %s | Model: %s | Offset: %.2f",
        args.drones,
        HEADLESS,
        LAUNCH_QGC,
        DETACH,
    ...
    try:
        ...
        log.info(
            "Tip: Headless training ⇒ keep --qgc false. Debugging ⇒ pass --qgc true and export $QGC_APPIMAGE."
        )
