...
def build_px4_command(
    ...
    if not px4_binary.exists():
        raise FileNotFoundError(
            f"PX4 binary not found at {px4_binary}. Build with 'make px4_sitl_default'."
        )
    if not rcS.exists():
        raise FileNotFoundError(
            f"Startup script not found at {rcS}. Rebuild px4_sitl_default to generate rootfs."
        )
    ...
    cmd = [
        ...
        "-d",
        "-s",
        "etc/init.d-posix/rcS",  # relative to <rootfs>
        "-w",
        str(rootfs_dir),  # working directory = build rootfs
...
def ensure_all_ready(log_paths: list[Path], timeout_sec: int) -> bool:
    ...
    for idx, log_path in enumerate(log_paths):
        ...
        if wait_for_ready(log_path, READY_PATTERNS, timeout_sec):
            ...
            logging.error(
                "PX4 #%d did NOT become ready within %ss; see %s", idx, timeout_sec, log_path
            )
            ok = False
    return ok
...
def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Launch PX4 SITL for Gazebo Harmonic on native Linux.")
    ...
    p.add_argument(
        ...
        "--autostart-id",
        type=int,
        default=DEFAULT_AUTOSTART_ID,
        help="PX4 airframe autostart ID (default: 4001)",
    )

    p.add_argument(
        "-m", "--model", default=DEFAULT_MODEL, help="Gazebo model name to spawn (default: x500)"
    )

    p.add_argument(
        "-p",
        "--px4-root",
        default=DEFAULT_PX4_ROOT,
