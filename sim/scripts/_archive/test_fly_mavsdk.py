...
async def wait_connected(drone, timeout=20):
    ...
    addr = os.getenv("MAVSDK_URL", "udpin://0.0.0.0:14540")
    print(f"[DEBUG] Using MAVSDK URL: {addr}", flush=True)
...
async def wait_position(drone, timeout=25):
    ...
    async for h in drone.telemetry.health():
        if h.is_global_position_ok or h.is_local_position_ok:
            print(
                f"[INFO] Position OK (global={h.is_global_position_ok}, local={h.is_local_position_ok})",
                flush=True,
            )
            return True
        if time.time() - t0 > timeout:
