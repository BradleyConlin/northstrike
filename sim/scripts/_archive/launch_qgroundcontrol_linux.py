...
if not APP or not os.path.isfile(APP):
    print("[ERROR] QGC_APPIMAGE not set or file missing.")
    sys.exit(1)
...
if try_launch(
    [APP, "--appimage-extract-and-run", "--clear-settings"], soft, "extract-and-run+software-GL"
):
    sys.exit(0)

print("[ERROR] QGC failed to start. Check GPU drivers/OpenGL; try foreground run with:")
print(
    '  QT_QPA_PLATFORM=xcb QT_OPENGL=software QSG_RHI_BACKEND=software "$QGC_APPIMAGE" --clear-settings'
)
sys.exit(2)
